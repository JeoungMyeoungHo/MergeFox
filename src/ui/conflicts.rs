...
enum ConflictIntent {
    ...
    /// Binary keep-both: resolve the conflict by taking `main` as the
    /// canonical file and writing the other side to a sibling path.
    /// The UI routes this to `Repo::resolve_conflict_keep_both` on a
    /// worker thread because large PSDs / archives are slow to write.
    KeepBoth {
        path: PathBuf,
        main: ConflictChoice,
    },
    ...
    /// default viewer (`open` / `xdg-open` / `explorer`). No backend
    /// state change; purely an aid for the user deciding which side
    /// to pick.
    OpenExternalSide {
        path: PathBuf,
        side: ConflictChoice,
