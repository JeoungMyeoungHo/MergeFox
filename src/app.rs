...
pub struct MergeFoxApp {
    ...
    ///
    /// Lives on `MergeFoxApp` (not `WorkspaceState`) because the
    /// fetcher runs on a worker thread and we don't want to pin the
    /// result to a specific tab — a user who switches tabs while a
    /// fetch is in flight still gets the badges painted when the
    /// task lands.
